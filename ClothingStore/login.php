...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand-headline {
    ...
    line-height: 1.2; color: var(--text); font-weight: 500;
  }
  .brand-headline em { font-style: italic; color: var(--accent); }
  .brand-desc { font-size: 14px; color: var(--muted); line-height: 1.7; max-width: 320px; }
  .brand-tags { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 0.5rem; }
  .tag {
  ...
  .form-title {
    font-family: 'Playfair Display', serif;
    font-size: 26px; font-weight: 500; color: var(--text); margin-bottom: 6px;
  }
  ...
  .btn {
    ...
    cursor: pointer; margin-top: 8px; background: var(--accent); color: #0d0d0d;
  ...
  .footer-link { text-align: center; margin-top: 1.5rem; font-size: 13px; color: var(--muted); }
...
<div class="form-panel">
  <div class="form-inner">
    ...
    <?php
    if(isset($_POST['login'])){
        $result2 = $conn->query("SELECT * FROM tblUser WHERE email='" . $_POST['email'] . "' AND password='" . md5($_POST['password']) . "'");
        if($result2 && $result2->num_rows > 0){
            $u = $result2->fetch_assoc();
